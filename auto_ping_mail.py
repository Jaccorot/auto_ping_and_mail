...
def mail_warn(error_ip, mail_address):
    ...
    发送模块
    '''
    content = ("Ping IP %s is error!It\'s done.Hurry"
               " up to restart,Jiecao Cao") % error_ip
    ...
    if error_ip == '100.0.0.86':
        try:
            ...
            s.sendmail(mail_address.mail_user,
                       [mail_address.mail_to, mail_address.mail_cc],
                       msg.as_string())
            s.close()
        except Exception, e:
            print e
    ...
    if error_ip != '100.0.0.86':
        try:
            ...
            s.login(mail_address.mail_user, mail_address.mail_pwd)
            s.sendmail(mail_address.mail_user,
                       [mail_address.mail_cc, mail_address.mail_cc],
                       msg.as_string())
            s.close()
        except Exception, e:
            print e
...
def check(get_ip):
    ...
    try:
        ...
        check_result = ping.expect(["Request", "(?i)time=",
